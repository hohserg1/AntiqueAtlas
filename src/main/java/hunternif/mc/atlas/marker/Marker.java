...
import static hunternif.mc.atlas.AntiqueAtlasMod.ieMarkerFilled;
...
public class Marker {
     ...
     */
    public String getTypeForRender() {
        if (FMLCommonHandler.instance().getEffectiveSide() == Side.CLIENT && type.equals(ieMarkerEmpty)) {
            if (label.contains(I18n.format("chat.immersiveengineering.info.coresample.noMineral")) && label.contains(I18n.format("chat.immersivepetroleum.info.coresample.noOil")))
                return type;
            else
                return ieMarkerFilled;
        } else
            return type;
    }
    ...
    public boolean isOfType(MarkerType type) {
        if (getTypeRaw() == null || type == null) return false;
        return getTypeRaw().equals(type.getRegistryName().toString());
    }
